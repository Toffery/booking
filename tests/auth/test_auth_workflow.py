...
async def test_auth_workflow(
    ...
    assert response_signup.status_code == status_code_signup, \
        ("Failed to signup", response_signup.status_code, response_signup.text)
    ...
    assert response_login.status_code == status_code_login, \
        ("Failed to login", response_login.status_code, response_login.text)
    ...
    assert response_me.status_code == 401, \
        ("Authorized after logout", response_me.status_code, response_me.text)
